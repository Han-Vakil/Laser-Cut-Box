...
current_y=0
current_x=0
def updent(drwing,ceerf,woood,x,y):
    drwing.add(drwing.line((x,y),(x,y-(ceerf+woood)),stroke=svgwrite.rgb(10, 10, 16, '%')))
...
def inner(inner_length,inner_width,inner_height,wood,cerf,nameo):
    ...
    for x in range(2):
        ...
        for x in range(int(len_number)//2):
            ...
            #dwg.add(dwg.line((current_x,current_y),(current_x,current_y-(cerf+wood)),stroke=svgwrite.rgb(10, 10, 16, '%')))
            #current_y=current_y-(cerf+wood)
            updent(dwg,cerf,wood,current_x,current_y)
            current_y=current_y-(cerf+wood)
...
def make_box(Name,length,width,height,kerf,mat_thickness):
    ...
    outer_width=width+mat_thickness*2+0.5
    outer_height=height+mat_thickness*2+0.5
    bottom(outer_length,outer_width,(float(outer_height*0.5)),mat_thickness,kerf,Name)
    #top(outer_length,outer_width,(float(outer_height)*0.5),mat_thickness,kerf,Name)
...
make_box("Card Box",18,65,91,0.16,3.4)
